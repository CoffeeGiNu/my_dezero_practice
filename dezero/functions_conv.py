...
class Im2col(Function):
    ...
    def forward(self, x):
        ...
        y = im2col_array(x, self.kernel_size, self.stride, self.padding,
                         self.to_matrix)
        return y

    def backward(self, gy):
        gx = col2im(gy, self.input_shape, self.kernel_size, self.stride,
                    self.padding, self.to_matrix)
        return gx

def im2col(x, kernel_size, stride=1, padding=0, to_matrix=True):
    ...
    """
    y = Im2col(kernel_size, stride, padding, to_matrix)(x)
    return y

class Col2im(Function):
    def __init__(self, input_shape, kernel_size, stride, padding, to_matrix):
        super().__init__()
    ...
    def backward(self, gy):
        gx = im2col(gy, self.kernel_size, self.stride, self.padding,
                    self.to_matrix)
        return gx

def col2im(x, input_shape, kernel_size, stride=1, padding=0, to_matrix=True):
    return Col2im(input_shape, kernel_size, stride, padding, to_matrix)(x)
...
def conv2d_simple(x, W, b=None, stride=1, padding=0):
    # WidthとWeightの競合を避ける
    ...
    # 入力データの展開
    col = im2col(x, (KH, KW), stride, padding, to_matrix=True)
    # カーネルを1列に展開
...
class Deconv2d(Function):
    ...
    def forward(self, x, W, b):
        ...
        gcol = xp.rollaxis(gcol, 3) # 次元の1番先頭に3次元目を持ってくる
        y = col2im_array(gcol, img_shape, (KH, KW), self.stride, self.padding,
                         to_matrix=False)
        # b, k, h, w
        if b is not None:
            self.no_bias = True
...
class Conv2DGradW(Function):
    ...
    def forward(self, x, gy):
        ...
        col = im2col_array(x, self.kernel_size, self.stride, self.padding,
                           to_matrix=False)
    ...
    def backward(self, gys):
        ...
        xh, xw = x.shape[2:]
        gx = deconv2d(gy, gW, stride=self.stride, padding=self.padding,
                      outsize=(xh, xw))
        ggy = conv2d(x, gW, stride=self.stride, padding=self.padding)
        return gx, ggy

def pooling_simple(x, kernel_size, stride=1, padding=0):
    x = as_variable(x)
    ...
    col = im2col(x, kernel_size, stride, padding, to_matrix=True) # 展開
    col = col.reshape(-1, KH * KW)
    y = col.max(axis=1) # 最大値を行ごとに求める
    y = y.reshape(N, OH, OW, C).transpose(0, 3, 1, 2) # 整形

...
class Layer:
    ...
    def save_weight(self, path):
        ...
        array_dict = {
            key: param.data for key, param in params_dict.items()
...
class Linear(Layer):
    def __init__(self, out_size, nobias=False, dtype=np.float32, in_size=None):
        ...
        if self.in_size is not None: # in_sizeが指定されている場合
            self._init_W()
...
class Conv2d(Layer):
    def __init__(self, out_channels, kernel_size, stride=1,
                 padding=0, nobias=False, dtype=np.float32, in_channels=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        y = F.conv2d(x, self.W, self.b, self.stride, self.padding)
        return y
